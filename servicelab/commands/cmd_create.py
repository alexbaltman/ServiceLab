...
def site_new(ctx, username, cont):
    ...
    if returncode > 0:
        click.echo("Error gathering site info.")
        return
    ...
    cmds = "cp -r ccs-build-tools/ignition_rb/sites/%(svc)s ccs-data/sites; " \
           "cp -r ccs-build-tools/host_generation/hosts.d/%(svc)s/. " \
           "ccs-data/sites/%(svc)s/environments/%(svc)s/hosts.d; " \
           "cp -r ccs-build-tools/host_generation/hosts.d/%(tc)s/. " \
           "ccs-data/sites/%(svc)s/environments/%(tc)s/hosts.d; " \
           "rm -rf ccs-build-tools/host_generation/hosts.d; " \
           "rm -rf ccs-build-tools/ignition_rb/sites; " \
           "cp ccs-build-tools/ignition_rb/answer-sample.yaml " \
           "ccs-data/sites/%(svc)s/data.d/answer-%(svc)s.yaml; " % {'svc': svc_site_name,
                                                                    'tc': tc_site_name
                                                                    }
...
def env_new(ctx, username):
    ...
    if passed > 0:
        ...
        return
    # Copying over contents of files generated by ccsbuildtools into ccs-data
    cmds = "cp -r ccs-build-tools/ignition_rb/sites/%(svc)s/environments/%(tc)s " \
           "ccs-data/sites/%(svc)s/environments; " \
           "rm -rf ccs-build-tools/ignition_rb/sites; " \
           "cp -r ccs-build-tools/host_generation/hosts.d/%(tc)s/. " \
           "ccs-data/sites/%(svc)s/environments/%(tc)s/hosts.d; " \
           "rm -rf ccs-build-tools/host_generation/hosts.d; " % {'svc': svc_site_name,
                                                                 'tc': tc_site_name
                                                                 }
    passed, log = service_utils.run_this(cmds, os.path.join(ctx.path, "services"))
    if passed > 0:
