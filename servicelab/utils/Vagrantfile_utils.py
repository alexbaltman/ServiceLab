...
class SlabVagrantfile(object):
    ...
    def init_vagrantfile(self):
        ...
        def _beg_vm_config():
            return "Vagrant.configure(VAGRANTFILE_API_VERSION) do |cluster|\n"

        h1, h2, req_plugin = _set_header()
        a = _beg_vm_config()
        self.write_it(h1, h2, req_plugin, a)
    ...
    def add_virtualbox_vm(self, host_dict):
        ...
        ip = host_dict.pop('ip')
        try:
            setitup = ("cluster.vm.define \"" + hostname + "\" do |config|\n",
                       "  config.vm.box = " + host_dict.pop('box') + "\n",
            ...
            for k, v in host_dict:
                if k in ['memory', 'cpus', 'mac']:
                    try:
            ...
            setitup += 'end\n'
            setitup += "  config.vm.hostname = \"" + hostname + "\"\n"
    ...
    def add_openstack_vm(self, host_dict):
        '''
        host_dict 'flavor', host_dict.get('image')
        '''
        hostname = self.host_dict.get('hostname')
    ...
    def _vbox_os_provider_host_vars(self, path):
        ...
        hostname = self.host_dict.pop('hostname')
        ...
        if (os.path.exists(path)):
            ...
            if (os.path.exists(path)):
                path = os.path.join(path, hostname.lower()+'.yaml')
                if os.path.exists(path):
                    try:
