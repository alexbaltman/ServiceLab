...
---------------------------------------
Ansible examples and Cisco usage links
---------------------------------------


This example represents a basic usage of Ansible for deployment

---
- hosts:all
launch task:
- name: check avail ping:
variables:
vars:
rack: green
include tasks:
...
Example 2: Complex Ansible Playbook:

This example represents a more complex usage of Ansible for deployment, including adding conditionals, and roles.

---
loop over items:
- name: copy file
copy: src={{item.src}} \
dest={{item.dst}} with_items:
- {src: a, dst: b}
- {src: k, dst: l}
conditionals:
- name: reboot Debian
...
Example 3: Highly complex, full featured Ansible Playbook:

This example represents a very complex usage of Ansible for deployment, with many features you will need to deploy your code.
...
Please click the link below to see more complex example of how Ansible is implemented withn the existing Cisco ecosystem.  Click on the gitweb link to the right side, then the tree link to see the file structure, then the blob link to view the code.

Note, you must be signed into cis-gerrit.cisco.com to view this information.

https://ccs-gerrit.cisco.com/#/admin/projects/?filter=ansible
