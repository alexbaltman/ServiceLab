...
class TestCCsBuildToolsUtils(unittest.TestCase):
    ...
    def setUp(self):
        ...
        """
        self.ctx = Context()
        service_utils.sync_service(self.ctx.path, "master", getpass.getuser(),
                                   "ccs-build-tools"
                                   )
    ...
    def TearDown(self):
        ...
        """
        shutil.rmtree(os.path.join(self.ctx.path, "services", "ccs-build-tools"))
    ...
    def test_buildargs_match_userinputs(self):
        ...
        data values used by ccsbuildtools.
           ...
           -> accessor function is called to assign site_by_user all the inputs needed by
              the current iteration of slab ccsbuildtools_utils
           -> the answer-sample.yaml file contains all the inputs needed by the current
              iteration of ccsbuild-tools to properly build a site
    ...
    def test_needed_directories_and_files_exist(self):
        ...
        the latest version of the repo
             -> host generation (where hosts are generated - need out of band vpn access)
             -> ignition (where site directory is made)
             -> Vagrantfile needed because site is generated using vagrant environment
        """
        path_to_repo = os.path.join(self.ctx.path, "services", "ccs-build-tools")
        self.assertEqual(os.path.isdir(os.path.join(path_to_repo, "host_generation")), True)
        self.assertEqual(os.path.isdir(os.path.join(path_to_repo, "ignition_rb")), True)
        self.assertEqual(os.path.isfile(os.path.join(path_to_repo, "Vagrantfile")), True)

    def test_exit_input(self):
        """test that the yaml dictionaries are properly dumped into file.
            -> test that the file exists
            -> tests that the file can be loaded into a dictionary, which is equivalent to
               the dictionary used to dump the file
        """
        site_by_user = ccsbuildtools_utils.get_input_requirements_for_ccsbuildtools()
        path_to_dump = os.path.join(self.ctx.path, "services", "ccs-build-tools", "a.yaml")
